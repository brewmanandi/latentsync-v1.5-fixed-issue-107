...
# Patched to avoid issue 107 - https://github.com/bytedance/LatentSync/issues/107 (face not detected)
...
class ImageProcessor:
    ...
    def __init__(self, resolution: int=512, mask: str='fix_mask', device: str='cpu', mask_image=None):
        ...
        self.resize = transforms.Resize((resolution, resolution), interpolation=transforms.InterpolationMode.BILINEAR, antialias=True)
        ...
        if mask == 'fix_mask':
            ...
            if device != 'cpu':
                try:
                    ...
                    print(f'Warning: face_alignment init failed ({e}), fallback to MediaPipe')
                    self.fa = None
                    self.face_mesh = mp.solutions.face_mesh.FaceMesh(static_image_mode=True)
                else:
                    self.face_mesh = None
    ...
    def detect_facial_landmarks(self, image: np.ndarray):
        ...
        landmark_coordinates = [(int(landmark.x * width), int(landmark.y * height)) for landmark in face_landmarks.landmark]
        return landmark_coordinates

    def preprocess_one_masked_image(self, image: torch.Tensor) -> np.ndarray:
        image = self.resize(image)
        if self.mask in ['mouth', 'face', 'eye', 'half']:
            np_img = image.permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            landmark_coordinates = self.detect_facial_landmarks(np_img)
    ...
    def affine_transform(self, image: torch.Tensor, allow_multi_faces: bool=True):
        ...
        if self.fa is None:
            ...
            landmark_coordinates = self.detect_facial_landmarks(np_img)
            if landmark_coordinates is None:
                print('No face detected (MediaPipe), using identity affine.')
                face = torch.nn.functional.interpolate(image.unsqueeze(0).float(), size=(self.resolution, self.resolution), mode='bilinear', align_corners=False).squeeze(0).to(torch.uint8)
                return (face, [0, 0, self.resolution, self.resolution], identity_affine)
            lm68 = mediapipe_lm478_to_face_alignment_lm68(np.array(landmark_coordinates))
        else:
            np_img = image.permute(1, 2, 0).cpu().numpy()
            detected_faces = self.fa.get_landmarks(np_img)
            if detected_faces is None:
                print('No face detected (FaceAlignment), using identity affine.')
                face = torch.nn.functional.interpolate(image.unsqueeze(0).float(), size=(self.resolution, self.resolution), mode='bilinear', align_corners=False).squeeze(0).to(torch.uint8)
                return (face, [0, 0, self.resolution, self.resolution], identity_affine)
        ...
        face, affine_matrix = self.restorer.align_warp_face(np_img.copy(), lmks3=lmk3_, smooth=True, border_mode='constant')
        box = [0, 0, face.shape[1], face.shape[0]]
        face = cv2.resize(face, (self.resolution, self.resolution), interpolation=cv2.INTER_LANCZOS4)
        face = rearrange(torch.from_numpy(face), 'h w c -> c h w')
        return (face, box, affine_matrix)

    def preprocess_fixed_mask_image(self, image: torch.Tensor, affine_transform: bool=False):
        if affine_transform:
    ...
    def prepare_masks_and_masked_images(self, images: Union[torch.Tensor, np.ndarray], affine_transform: bool=False):
        if isinstance(images, np.ndarray):
            images = torch.from_numpy(images)
        if images.ndim == 4 and images.shape[3] == 3:
            images = rearrange(images, 'f h w c -> f c h w')
        if self.mask == 'fix_mask':
            results = [self.preprocess_fixed_mask_image(image, affine_transform) for image in images]
        else:
...
mouth_surround_landmarks = [164, 165, 167, 92, 186, 57, 43, 106, 182, 83, 18, 313, 406, 335, 273, 287, 410, 322, 391, 393]
face_surround_landmarks = [152, 377, 400, 378, 379, 365, 397, 288, 435, 433, 411, 425, 423, 327, 326, 94, 97, 98, 203, 205, 187, 213, 215, 58, 172, 136, 150, 149, 176, 148]
