...
class laplacianSmooth(object):
    ...
    def smooth(self, points):
        if self.prev_points is None:
            ...
            return points
        smoothed_points = self.smoothing_factor * points + (1 - self.smoothing_factor) * self.prev_points
...
class AlignRestore(object):
    ...
    def align_warp_face(self, img, lmks3, smooth=True, border_mode='constant'):
        ...
        cropped_face = kornia.geometry.transform.warp_affine(img, affine_matrix, (self.face_size[1], self.face_size[0]), mode='bilinear', padding_mode=padding_mode, fill_value=self.fill_value)
    ...
    def restore_img(self, input_img, face, affine_matrix):
        ...
        if isinstance(affine_matrix, np.ndarray):
            affine_matrix = torch.from_numpy(affine_matrix).to(device=self.device, dtype=self.dtype).unsqueeze(0)
        check_matrix = affine_matrix.squeeze()
        target_identity = torch.tensor([[1, 0, 0], [0, 1, 0]], device=self.device, dtype=self.dtype)
        if torch.sum(torch.abs(check_matrix - target_identity)) < 0.001:
            return input_img
        ...
        inv_face = kornia.geometry.transform.warp_affine(face, inv_affine_matrix, (h, w), mode='bilinear', padding_mode='fill', fill_value=self.fill_value).squeeze(0)
        ...
        inv_mask = kornia.geometry.transform.warp_affine(self.mask, inv_affine_matrix, (h, w), padding_mode='zeros')
        inv_mask_erosion = kornia.morphology.erosion(inv_mask, torch.ones((int(2 * self.upscale_factor), int(2 * self.upscale_factor)), device=self.device, dtype=self.dtype))
    ...
    def transformation_from_points(self, points1: torch.Tensor, points0: torch.Tensor, smooth=True, p_bias=None):
        ...
        if smooth:
            if points2_normalized.shape[0] > 2:
